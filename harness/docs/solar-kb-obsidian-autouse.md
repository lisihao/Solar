...
2. **Obsidian vault indexing** — `${SOLAR_KNOWLEDGE_DIR}` is indexed into
...
```bash
solar-harness wiki sync-vault --vault ${SOLAR_KNOWLEDGE_DIR} --once
```
...
```bash
# Re-index vault
solar-harness wiki sync-vault --vault ${SOLAR_KNOWLEDGE_DIR}
