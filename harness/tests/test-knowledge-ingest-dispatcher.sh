...
raw_out="$(python3 "$DISPATCHER" --db "$DB" discover-raw --source-dir "$TMP_DIR/raw" --span-root "$TMP_DIR/spans" --json 2>&1)"
...
vault_out="$(python3 "$DISPATCHER" --db "$DB" discover-vault --vault "$TMP_DIR/vault" --span-root "$TMP_DIR/spans" --json 2>&1)"
