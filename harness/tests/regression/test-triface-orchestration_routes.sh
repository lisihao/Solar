#!/usr/bin/env bash
# test-triface-orchestration_routes.sh — triface regression wrapper for status-server/routes/orchestration_routes.py
#
# Runs the real Python test that imports and exercises orchestration_routes.
# Writes a schema-compliant JSON report to reports/s05/regression/orchestration_routes.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/orchestration_routes.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-orchestration_routes.py"
MODULE="orchestration_routes"
