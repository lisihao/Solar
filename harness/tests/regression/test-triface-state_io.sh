#!/usr/bin/env bash
# test-triface-state_io.sh — triface regression wrapper for lib/task_graph_state_io.py
#
# Runs the real Python test that imports and exercises task_graph_state_io.
# Writes a schema-compliant JSON report to reports/s05/regression/state_io.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/state_io.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-state_io.py"
MODULE="state_io"
