#!/usr/bin/env bash
# test-triface-task_graph_io.sh — triface regression wrapper for lib/task_graph_io.py
#
# Runs the real Python test that imports and exercises task_graph_io.
# Writes a schema-compliant JSON report to reports/s05/regression/task_graph_io.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/task_graph_io.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-task_graph_io.py"
MODULE="task_graph_io"
