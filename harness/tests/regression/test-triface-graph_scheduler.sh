#!/usr/bin/env bash
# test-triface-graph_scheduler.sh — triface regression wrapper for lib/graph_scheduler.py
#
# Runs the real Python test that imports and exercises graph_scheduler.
# Writes a schema-compliant JSON report to reports/s05/regression/graph_scheduler.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/graph_scheduler.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-graph_scheduler.py"
