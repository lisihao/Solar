#!/usr/bin/env bash
# test-triface-contract_closure.sh — triface regression wrapper for lib/contract_closure.py
#
# Runs the real Python test that imports and exercises contract_closure.
# Writes a schema-compliant JSON report to reports/s05/regression/contract_closure.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/contract_closure.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-contract_closure.py"
MODULE="contract_closure"
