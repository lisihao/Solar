#!/usr/bin/env bash
# test-triface-multi_task_status.sh — triface regression wrapper for tools/multi_task_status.py
#
# Runs the real Python test that imports and exercises multi_task_status.
# Writes a schema-compliant JSON report to reports/s05/regression/multi_task_status.json.
set -uo pipefail

HARNESS_DIR="${HARNESS_DIR:-$HOME/.solar/harness}"
REPORT_DIR="$HARNESS_DIR/reports/s05/regression"
REPORT_FILE="$REPORT_DIR/multi_task_status.json"
PY_TEST="$HARNESS_DIR/tests/regression/test-triface-multi_task_status.py"
MODULE="multi_task_status"
