...
def test_enqueue_ready_plan_artifacts_has_all_8_apo_keys(tmp_path):
    ...
    with patch("apo_plan_compiler.compile_execution_plan_for_node", return_value=compiled), \
         patch("apo_plan_compiler.materialize_execution_plan_artifacts", return_value={
             "capsule_plan_ir_path": "/fake/cap.json",
             "physical_plan_ir_path": "/fake/phys.json"
         }), \
         patch("task_queue.enqueue") as mock_enqueue:

        sched.enqueue_ready(graph, graph_path, graph["workers"], lease=False, dry_run=False)

        assert mock_enqueue.called
        payload = mock_enqueue.call_args[0][3]
        plan_artifacts = payload.get("plan_artifacts")

        for key in APO_PLAN_ARTIFACT_KEYS:
            assert key in plan_artifacts, f"Missing APO key in plan_artifacts: {key}"
            assert plan_artifacts[key] == compiled[key]
...
def test_ready_nodes_releases_expired_repeated_transient_block(monkeypatch):
    import graph_scheduler as sched

    monkeypatch.setattr(sched, "TRANSIENT_FAILURE_BLOCK_TTL_SEC", 3600)
    graph = {
        "sprint_id": "expired-transient-block",
        "nodes": [
            {
                "id": "S1",
                "depends_on": [],
                "status": "pending",
                "blocking_reason": "repeated_transient_operator_failure",
                "transient_failure_blocked_at": "2026-01-01T00:00:00Z",
                "acceptance": ["done"],
            },
            {
                "id": "S2",
                ...
                "status": "pending",
                "blocking_reason": "repeated_transient_operator_failure",
                "transient_failure_blocked_at": sched._now(),
                "acceptance": ["done"],
            },
            {
                "id": "S3",
