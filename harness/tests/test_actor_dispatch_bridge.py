...
class TestContextPacketLoad:
    ...
    def test_inline_context_packet_when_ref_absent(self):
        """build_envelope must include inline task context when node lacks a stored ref."""
        from actor_dispatch_bridge import build_envelope

        node = _sample_node(
            acceptance=["pytest exits 0"],
            stop_rules=["do not touch unrelated files"],
        ...
        assert envelope["context_packet"]["data"]["sprint_id"] == "sprint-test"
        assert envelope["context_packet"]["data"]["node_id"] == "N_test_node"
        assert envelope["context_packet"]["data"]["goal"] == "Test goal"
        assert envelope["context_packet"]["data"]["read_scope"] == ["lib/"]
        assert envelope["context_packet"]["data"]["write_scope"] == ["tests/"]
        assert envelope["context_packet"]["data"]["acceptance"] == ["pytest exits 0"]
        assert envelope["context_packet"]["data"]["stop_rules"] == ["do not touch unrelated files"]
        assert envelope["context_packet"]["data"]["required_capabilities"] == ["python", "testing"]

    def test_existing_inline_context_forwarded(self):
        """build_envelope must preserve explicit context_packet supplied by graph node."""
        from actor_dispatch_bridge import build_envelope

        context_packet = {"packet_type": "task", "data": {"node_id": "custom"}}
        node = _sample_node(context_packet=context_packet)
        envelope = build_envelope("sprint-test", node)
        assert envelope.get("context_packet") == context_packet

    def test_context_policy_forwarded(self):
        """build_envelope must forward explicit context policy metadata."""
        from actor_dispatch_bridge import build_envelope

        node = _sample_node(context_policy="critical", expected_packet_type="task")
        envelope = build_envelope("sprint-test", node)
        assert envelope["context_policy"] == "critical"
        assert envelope["expected_packet_type"] == "task"
...
class TestDispatchBridgeFingerprintPenalty:
    def test_submit_computes_and_retains_fingerprint_penalty(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            assert res.scheduler_decision["failure_fingerprint"]["penalty"] > 0
            assert res.scheduler_decision["failure_fingerprint"]["matched_labels"] == ["shallow_final_reasoning"]
            assert res.scheduler_decision["failure_fingerprint"]["evidence_refs"] == ["ev-1"]
            assert "FINAL_REVIEW" in res.scheduler_decision["failure_fingerprint"]["explanation"]
