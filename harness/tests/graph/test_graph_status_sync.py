...
def test_sync_status_cache_repairs_stale_task_graph_status_for_passed_parent(tmp_path, monkeypatch):
    ...
    stale_status = {
        ...
        "acceptance_verdict": {"blocked": True, "verdict": "FAIL"},
        "history": [],
    }
    ...
    assert "acceptance_verdict" not in updated


def test_sync_status_cache_clears_stale_acceptance_projection_for_passed_parent(tmp_path, monkeypatch):
    import graph_scheduler as gs
    ...
    sid = "sprint-test-stale-acceptance-projection"
    graph_path = sprints / f"{sid}.task_graph.json"
    status_path = sprints / f"{sid}.status.json"
    graph = {
        "sprint_id": sid,
...
def test_sync_status_cache_refreshes_pending_closure_before_parent_pass(tmp_path, monkeypatch):
    ...
    sid = "sprint-test-pending-closure"
    graph_path = sprints / f"{sid}.task_graph.json"
    status_path = sprints / f"{sid}.status.json"
    ...
    closure_path.write_text(
        json.dumps({"status": "pending", "all_nodes_passed": True, "all_required_gates_passed": True}),
        encoding="utf-8",
    )
    ...
    assert result["ok"] is True
    assert result["closure_projection"]["updated"] is True
    updated = json.loads(status_path.read_text(encoding="utf-8"))
...
def test_sync_status_cache_does_not_reopen_passed_sprint_with_terminal_evidence(tmp_path, monkeypatch):
    import graph_scheduler as gs
    ...
    sid = "sprint-test-terminal-evidence"
    graph_path = sprints / f"{sid}.task_graph.json"
    status_path = sprints / f"{sid}.status.json"
    ...
    graph = {
        "sprint_id": sid,
        "title": "Terminal evidence child",
        "created_at": "2026-05-31T20:00:00Z",
        "required_gates": ["G4"],
        "nodes": [
            {"id": "R1", "status": "passed", "depends_on": [], "gate": "G1"},
            {"id": "R2", "status": "pending", "depends_on": ["R1"], "gate": "G4"},
        ],
        "node_results": {
            "R1": {"status": "passed"},
        },
    ...
    result = gs.sync_status_cache_from_graph(graph, graph_path, actor="test", event="graph_parent_ready_revoked")

    updated = json.loads(status_path.read_text(encoding="utf-8"))
    assert result["ok"] is True
    assert result["updated"] is True
    assert result["reason"] == "terminal_evidence_preserved"
    assert updated["status"] == "passed"
...
def test_parent_ready_check_self_heals_ownerless_gate_only_with_node_evidence(tmp_path, monkeypatch):
    import graph_scheduler as gs
    ...
    sid = "sprint-test-ownerless-gate"
    graph = {
        "sprint_id": sid,
        "required_gates": ["G_IMPL", "G_VERIFY"],
        "nodes": [
            {"id": "N1", "status": "passed", "gate": "G_IMPL", "depends_on": []},
        ],
        "gate_results": {},
    }
    ...
    assert result["missing_gates"] == ["G_VERIFY"]
    assert "G_VERIFY" not in graph["gate_results"]

    (sprints / f"{sid}.N1-handoff.md").write_text("# handoff\n", encoding="utf-8")
    (sprints / f"{sid}.N1-eval.json").write_text('{"verdict":"PASS"}\n', encoding="utf-8")

    result = gs.parent_ready_check(graph)

    assert result["ready"] is True
    assert result["missing_gates"] == []
    assert graph["gate_results"]["G_VERIFY"]["status"] == "passed"
    assert graph["gate_results"]["G_VERIFY"]["reason"] == "parent_ready_ownerless_gate_self_heal"
