...
> **harness/engine 内核不得承载"可换 backend"横切实现**——telemetry exporter / sandbox runtime / cache backend 等真正存在多种竞争实现的能力，必须以 plugin 形态存在于 `src/plugins/`，通过 plugin-core 提供的 HookBus 注入 harness/engine。

### Plugin 抽取规则（2026-05-04 二次修正后定稿）

**核心思想：反应式抽取，不预抽**。代码库默认所有能力按核心 service / middleware 实现；只有当**真实 backend 替换需求出现**时，才抽 plugin 形态。

**3 条充要条件（必须全部满足才抽 plugin）**：

| #   | 条件                                      | 反例                                                               |
| --- | ----------------------------------------- | ------------------------------------------------------------------ |
| 1   | **代码库内已存在 ≥ 2 种 backend 实现**    | 单一 `embedding.service.ts`（即使 LiteLLM 内部多 provider）→ 不抽  |
| 2   | **替换是部署期决策（不是 runtime 路由）** | TaskProfile 路由不同模型 → 不抽（runtime 路由是 service 内部细节） |
| 3   | **实现差异是结构性的（不是参数）**        | redis cluster vs single-node 是参数 → 不抽                         |

**3 条禁止条件（满足任一不抽 plugin）**：

| #   | 禁止情况                        | 解释                                                                          |
| --- | ------------------------------- | ----------------------------------------------------------------------------- |
| 1   | 一个标准算法 / 状态机有公认实现 | token-bucket / circuit-breaker / semaphore / setTimeout race / zod schema     |
| 2   | 跨 ai-app 共用 ≠ plugin         | 平台公共 service 跨 app 用是常态（rate-limit / pii / pricing），不等于 plugin |
| 3   | 协议层差异（SSE vs WebSocket）  | 是 transport 不是 backend，归 middleware                                      |

**反应式抽取流程**：

1. 实际场景出现"我要用 X 替换 Y"（如客户要 Qdrant 替 pgvector）
2. 写一个 1 周内必交付的 ADR（≤ 1 页）记录驱动需求 + 接口设计
3. 抽 IXxxBackend 端口 + 把现有实现搬成第一个 backend + 加新 backend
4. **同 PR 落地**：不允许"先抽接口、二期再加新 backend"

**禁止行为**：

- ❌ 因为"v5.1 plan §11.1 列了 X plugin"就抽 X，没有真实替换需求
- ❌ "为未来 SDK 发布做准备"提前抽接口（YAGNI）
- ❌ 把单实现 service 改成 plugin 形态"以提升可测试性"（mock/stub 用 jest 即可）

**反例清单（不该是 plugin）**：

| 能力                          | 为什么不是 plugin                    | 应归                 |
| ----------------------------- | ------------------------------------ | -------------------- |
| tool-timeout                  | race + setTimeout 一个标准实现       | ai-engine middleware |
| tool-validation-zod           | zod schema 校验一个标准实现          | ai-engine middleware |
| circuit-breaker               | CLOSED/OPEN/HALF_OPEN 一个标准状态机 | ai-engine service    |
| rate-limit                    | token-bucket 一个标准算法            | ai-engine service    |
| concurrency-control           | semaphore 一个标准模式               | ai-engine service    |
| budget-billing                | 计费规则业务无关，但实现单一         | ai-harness service   |
| guardrail-injection           | prompt-injection 检测无 backend 替换 | ai-engine service    |
| llm-prompt-adapter            | 统一适配所有 provider 即标准实现     | ai-engine service    |
| llm-output-sanitizer          | 统一规范化即标准实现                 | ai-engine service    |
| tool-progress-sse / websocket | 协议层差异，不是 plugin 替换语义     | ai-engine middleware |
...
|           | E-W1-a tool-augment 域 plugin 迁出（已撤销）                      | ⏪ reverted | `34a4c7cf5`             | tool-timeout / tool-validation-zod 不属"可换 backend"，回归 ai-engine 核心 middleware；rate-limit scaffold 同步删除（重新归位 `71afe9a6a` 为 ai-engine 核心 service）       |
|           | E 整体规划（R0.5-E "22 plugin 批量迁出"）                         | ❌ scrapped | —                       | 二次修正后确认：剩余原计划 plugin 在代码库内多为单一 backend 实现，预抽=过度抽象。改为**反应式抽取**（实际 backend 替换需求出现时单 plugin PR 落地，详见 §11.0 修正后规则） |
...
|           | A.14 mission_runs.metadata.runtime_version 列 + 双轨产物对比工具  | ⏪ moot     | (R2-C 后无意义)         | R2-C 单轨化后 legacy 不存在，对比工具不必要；DB 列保留无害（commit 后续修正默认值写 "pipeline-v1"）                                                                         |
| **R2-B**  | e2e 等价性验证 + 1 周双轨观察                                     | ⏪ moot     | —                       | R2-C 直接单轨化（无 legacy 可对比），用线上观察替代                                                                                                                         |
| **R2-C**  | 删除 legacy team.mission.ts + 14 stage + duty-loader + 18 .md     | ✅          | `27350f494`             | 6253 行 team.mission + flag service + 路由 spec 全删；保留 14 stage 函数（pipeline hook thin-adapter 调用）                                                                 |
...
| R2-A.14 双轨产物对比工具                | ⏪ moot       | R2-C 后无意义  |
| R2-B 双轨等价 1 周观察通过              | ⏪ moot       | R2-C 直接单轨  |
| R2-C legacy 删除完成                    | ✅ 2026-05-04 | `27350f494`    |
| Phase 2 用户自定义 agent UI 上线        | ⏳ pending    | Phase 1 稳定后 |

#### 主干安全状态（截至 2026-05-04 R2-C 单轨化完成）

- ★ **pipeline-v1 是唯一 mission 路径**（R2-C `27350f494` 删除 legacy team.mission + flag service）
- ⚠️ 紧急回滚已无 legacy 可切：如 pipeline-v1 出现 P0，需 git revert 回 R2-C 之前的 commit + redeploy
- 14 stage 函数文件保留（s1-s12 stage.ts），pipeline-v1 hook 通过 thin-adapter 调用
- runtime_version DB 列保留无害（migration 已 deploy prod 无法 down，但写值固定 "pipeline-v1"）
- 主干现有 R0.5/R1/R3-A/R2-A0/R2-A 全量推到 prod，单轨流量稳定运行
...
**目标**：内核接口就位 + 关键路径打 hook + 3 个真 plugin 落地（telemetry-otel / tool-cache-redis / sandbox-isolated-vm，验证可行性）。

> **2026-05-04 修正**：原 R0.5-E "剩余 22 个 plugin 批量迁出"已 ❌ scrapped。剩余能力**不批量预抽**，按§11.0 "反应式抽取规则"按需做单 plugin PR。
...
★ R0.5-E（R1 完成后做，不阻塞主线）：~~剩余 22 个 plugin 批量迁出~~ ❌ scrapped 2026-05-04 — 按反应式规则单 plugin 按需抽取

Phase 1 总计 16 周（4 个月，v5 估 14 周 → v5.1 +2 周）
Phase 2 总计 4 周
~~R0.5-E 4-6 周~~ ❌ scrapped — 反应式 plugin 抽取，无固定排期
含评审 + 与用户决策来回 buffer 1-2 周
全部完成：18-22 周（4.5-5.5 个月，R0.5-E 已 scrapped）
```
...
## §11 Plugin 系统（v5 新增 / v5.1 评审收敛 / 2026-05-04 反应式规则修正）

> **目标（修正后）**：把 harness/engine 中**真正存在多 backend 替换需求**的横切关注点剥离到可插拔 plugin。其余横切能力归核心 service / middleware。
>
> **不在范围**：plugin marketplace（v3 之后）/ plugin hot-reload（v2）/ worker-isolated 沙箱（v2）。本章只覆盖 v1 必须落地的范围。

### §11.0 Plugin 抽取规则（**强制规则 / 2026-05-04 二次修正后定稿**）

经过两次踩坑（W1-a tool-timeout/validation 误抽 + W1-b circuit-breaker 误抽），plugin 抽取规则定稿如下。**任何新增 plugin 提案必须按本节规则评估**，否则 PR 直接拒绝。

#### A. 充要条件（**必须全部满足**）

| #   | 条件                                   | 不满足 = 不抽 plugin                                                       |
| --- | -------------------------------------- | -------------------------------------------------------------------------- |
| 1   | **代码库内已存在 ≥ 2 种 backend 实现** | 单 backend 抽接口 = YAGNI；先有第二实现再抽                                |
| 2   | **替换是部署期决策，非 runtime 路由**  | TaskProfile 路由不同模型属 service 内部细节，不抽                          |
| 3   | **实现差异结构性（非参数化）**         | redis cluster vs single-node 是参数 / 不抽；vm2 vs isolated-vm 是结构 / 抽 |
...
| --- | ------------------------------------------ | ----------------------------------------------------------------------------------- |
| 1   | 一个标准算法 / 状态机有公认实现            | token-bucket / circuit-breaker / semaphore / setTimeout race / zod schema           |
| 2   | 跨 ai-app 共用 ≠ plugin（旧第 2 条已作废） | rate-limit / pricing / capability-guard / pii-detector — 凡平台 service 都跨 app 用 |
| 3   | 协议 transport 层差异                      | SSE vs WebSocket、gRPC vs HTTP — 归 middleware/transport                            |

#### C. 反应式抽取流程

1. 实际场景出现"我要用 X 替换 Y"（如客户要 Qdrant 替 pgvector / 接 Llama Guard）
2. 写 ADR ≤ 1 页：驱动需求 + 接口设计 + 第二 backend 工作量
3. **同 PR 落地**：抽 IXxxBackend 端口 + 现有实现包成第一 backend + 新 backend + spec
4. ❌ 禁止"先抽接口、二期补 backend"

#### D. 历史决策追溯

| 时间                | 行动                                  | 教训                                |
| ------------------- | ------------------------------------- | ----------------------------------- |
| v5.0 (2026-04)      | 列 26 plugin 计划                     | 以"跨 app 共用"为入选标准，过度抽象 |
| 2026-05-04 W1-a     | 抽 tool-timeout / validation → 即回滚 | 标准实现不该抽 plugin               |
| 2026-05-04 W1-b     | 抽 circuit-breaker → 半路停           | 同上，状态机一标准                  |
| 2026-05-04 二次修正 | 定 §11.0 规则 + R0.5-E scrapped       | 规则替代清单                        |

#### E. 当前 plugin 名册（2026-05-04 锁定）

**真 plugin（已落地，3 个）**：

- `observability/telemetry-otel` — backend：OTel / Datadog / Sentry / disable
- `storage/tool-cache-redis` — backend：Redis / Memcached / in-memory
- `security/sandbox-isolated-vm` — backend：vm2 / isolated-vm / worker / disable

**未来候选（不预抽）**：见 §11.1 "未来候选" 表，触发条件 = 实际 backend 替换需求出现。
...
### §11.1 设计目标 5 维 + plugin 实际盘点（**2026-05-04 二次修正**）
...
| 域            | plugin id                      | 替换 backend                         | 状态                 |
| ------------- | ------------------------------ | ------------------------------------ | -------------------- |
| observability | `observability/telemetry-otel` | OTel / Datadog / Sentry / disable    | ✅ scaffold（PR-7）  |
| storage       | `storage/tool-cache-redis`     | Redis / Memcached / in-memory        | ✅ scaffold（PR-8）  |
| security      | `security/sandbox-isolated-vm` | vm2 / isolated-vm / worker / disable | ✅ scaffold（PR-10） |

#### 历史误分类清单（不再做 plugin，已回归核心）

> v5 原计划"26 plugin"是对插件抽象的过度应用。下列能力已确认归核心 service / middleware，不走 HookBus：

| ~~原 plugin id~~                               | 实际归位                                                      | 说明                         |
| ---------------------------------------------- | ------------------------------------------------------------- | ---------------------------- |
| ~~tool-augment/tool-timeout~~                  | `engine/tools/middleware/timeout.middleware.ts`               | race + setTimeout 一标准实现 |
| ~~tool-augment/tool-validation-zod~~           | `engine/tools/middleware/validation.middleware.ts`            | zod schema 一标准实现        |
| ~~resilience/rate-limit~~                      | `engine/safety/resilience/rate-limit.service.ts`              | token-bucket 一标准算法      |
| ~~resilience/circuit-breaker~~                 | `engine/safety/resilience/circuit-breaker.service.ts`         | 状态机一标准                 |
| ~~resilience/concurrency-control~~             | `engine/tools/concurrency/tool-concurrency.service.ts`        | semaphore 一标准             |
| ~~resilience/budget-billing~~                  | `harness/guardrails/billing/`                                 | 业务统计一标准               |
| ~~security/tool-permission-rbac~~              | `engine/tools/middleware/permission.middleware.ts`            | RBAC 一标准                  |
| ~~security/capability-guard-rbac~~             | `engine/safety/security/capability-guard.service.ts`          | 能力门一标准                 |
| ~~security/guardrail-injection~~               | `engine/safety/guardrails/input/prompt-injection-detector.ts` | detector 一标准              |
| ~~observability/telemetry-latency~~            | `harness/tracing/latency/`                                    | sliding window 统计          |
| ~~observability/telemetry-llm-events~~         | `harness/tracing/observability/llm-events.listener.ts`        | listener 桥接                |
| ~~observability/telemetry-attribution~~        | `harness/tracing/observability/cost-attribution.service.ts`   | cost 归因规则                |
| ~~llm-augment/llm-multi-key~~                  | `engine/llm/key-health/`                                      | health-based 选择策略        |
| ~~llm-augment/llm-pricing~~                    | `engine/llm/pricing/`                                         | 价格 lookup 一标准           |
| ~~llm-augment/llm-prompt-adapter~~             | `engine/llm/prompt-adaptation/`                               | provider 统一 adapter        |
| ~~llm-augment/llm-output-sanitizer~~           | `engine/llm/output-parsing/`                                  | 统一规范化                   |
| ~~tool-augment/tool-progress-sse / websocket~~ | `engine/tools/middleware/progress.middleware.ts`              | 协议 transport 一致          |

#### 未来候选（**不预抽**，反应式）

下列能力**可能**未来需要 plugin 化（当出现真实 backend 替换需求时）。**不做预抽**：

| 候选域        | 候选能力                                                                | 抽 plugin 触发条件                                   |
| ------------- | ----------------------------------------------------------------------- | ---------------------------------------------------- |
| storage       | `memory-redis` / `memory-postgres` / `checkpoint-*` / `event-journal-*` | 当出现非默认 storage 部署需求（如客户用 DynamoDB）   |
| rag-backend   | `embedding-*` / `vector-*`                                              | 当代码库实际加入第二个 backend（如 Qdrant / Voyage） |
| security      | `guardrail-pii-moderation`                                              | 当客户要接 Llama Guard / Azure Moderation            |
| observability | `telemetry-eval`                                                        | 当接入 LangSmith / Braintrust / 其他 eval framework  |

**触发流程**：实际需求 → ADR ≤ 1 页 → 同 PR 内（接口 + 第一个 backend + 新 backend）一次落地。**不允许"先抽接口，二期补 backend"。**
...
#### Stage 4：~~剩余 22 plugin 批量迁出（R0.5-E）~~ ❌ scrapped 2026-05-04

**作废原因**：原"批量迁出"假设的待迁能力在代码库内多数仅 1 个 backend 实现（如 `embedding.service.ts` 单文件 / `vector.service.ts` 单文件），预抽 plugin 形态 = 提前抽接口 = YAGNI 反模式，已踩过 W1-a / W1-b 同类陷阱。

**改为反应式抽取**（详见 §11.0 修正后规则）：

1. 当代码库内**实际**出现 ≥ 2 种 backend 实现需求（驱动场景：客户要 Qdrant 替 pgvector / 接 LangSmith 替本地 eval / 接 Llama Guard 替简单 detector）时
2. 写 ADR ≤ 1 页记录驱动需求 + 接口
3. 单 plugin PR：抽接口 + 现有实现包成第一 backend + 新 backend 实现 + spec
4. **不允许"先抽接口、二期补 backend"**

候选清单（参考用，不主动推进）：
`embedding-*` / `vector-*` / `memory-*` / `checkpoint-*` / `event-journal-*` /
`guardrail-pii-moderation` / `telemetry-eval`
...
~~R0.5-E（R1 后做）~~ ❌ scrapped    反应式 plugin 抽取（按真实 backend 替换需求触发）
R2/R3/R4 不变
```
