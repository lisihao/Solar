...
def build_consumer_text(raw: dict[str, Any], rewritten: dict[str, Any], ir: dict[str, Any]) -> str:
    ...
    source_inputs = ir.get("source_inputs") if isinstance(ir.get("source_inputs"), dict) else {}
    enhanced_requirement = source_inputs.get("enhanced_requirement") if isinstance(source_inputs.get("enhanced_requirement"), dict) else None
    ...
    if enhanced_content:
        lines.extend([
            ...
            enhanced_content,
            "",
            "Use this enhanced design as requirement context, but keep the final PRD traceable to Raw User Intent and explicit acceptance criteria.",
            "",
        ])
    lines.extend([
        "## Raw User Intent",
