...
__all__ = ["DatasetLoader", "load_dataset", "case_raw_intent", "iter_runner_inputs"]


def case_raw_intent(case: EvalCase) -> str:
    """Derive the raw requirement text for an EvalCase, artifact-first.

    Priority chain (real data only, no synthesis):
      1. ``requirement_ir.source_inputs.raw_request`` / ``intent`` / ``goal``
      2. fallback ``input_text`` (accepted.md heuristic)
      3. handoff content (real builder handoff text)
      4. eval content (real evaluator text)
      5. first contract's intent/description field

    Returns ``""`` only when the case carries no usable text at all.
    """
    ir = case.requirement_ir
    if isinstance(ir, dict):
        si = ir.get("source_inputs")
        if isinstance(si, dict) and si.get("raw_request"):
            return str(si["raw_request"]).strip()
        for key in ("raw_request", "intent", "goal", "description"):
            if ir.get(key):
                return str(ir[key]).strip()
    ...
    if isinstance(handoff, dict) and handoff.get("content"):
        return str(handoff["content"]).strip()

    ev = case.eval
    if isinstance(ev, dict) and ev.get("content"):
        return str(ev["content"]).strip()

    if case.contracts:
        first = case.contracts[0]
        if isinstance(first, dict):
            for key in ("intent", "description", "goal"):
                if first.get(key):
...
def iter_runner_inputs(cases: list[EvalCase]) -> list[tuple[str, str]]:
    """Bridge loader → runner: yield ``(case_id, raw_intent)`` pairs.

    Cases with no derivable text are skipped (they cannot drive the
    compile-then-evaluate loop).  The returned pairs feed directly into
    ``eval_runtime.compile_and_evaluate(profile, raw_intent, case_id=…)``.
    """
    pairs: list[tuple[str, str]] = []
    for case in cases:
        raw = case_raw_intent(case)
        if raw:
            pairs.append((case.case_id, raw))
    return pairs
