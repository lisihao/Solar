...
def _builtin_default_profile() -> dict[str, Any]:
    ...
    """
    # The built-in fallback is a *runtime* artifact (it can be compiled when no
    # other profile resolves), so it must be strict-v2 valid: every policy
    # carries a non-empty deterministic text body.
    policies = {
        key: {
            "version": "1",
            "params": {},
            "text": f"Built-in default {key.replace('_', ' ')} body.",
        }
        for key in (
            "intake_policy",
