...
STDERR_LOG="${LOG_DIR}/operator-recovery-scanner.err.log"
...
usage() {
  cat <<EOF
Solar Harness operator recovery scanner daemon
...
Environment:
  SOLAR_OPERATOR_RECOVERY_SCANNER_INTERVAL  Default: 600
  SOLAR_OPERATOR_RECOVERY_SCANNER_LABEL     Default: ${LABEL}
EOF
}
...
run_once() {
  mkdir -p "$LOG_DIR"
  printf '[%s] operator-recovery-scanner start\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  python3 "${HARNESS_DIR}/tools/operator_recovery_scanner.py" --apply --json
  printf '[%s] operator-recovery-scanner end\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
}
...
status_job() {
  ...
  if command -v launchctl >/dev/null 2>&1; then
    launchctl print "${domain}/${LABEL}" 2>/dev/null | head -40 || echo "launchd_state=not_loaded"
  fi
}
